...
# play sound
#----------------------------------------------------------------------
def play_sound(person, content):
	root_dir = 'C:\Users\Yu\Documents\Gavin\python\Gobang\gobang\sound' + '\\' + person + '\\'
	sound_file = content + '.m4a'
	sys_command = "start " + root_dir + sound_file
...
def gamemain():
	...
	#import random
	openid = 0 #random.randint(0, len(opening) - 1)
	...
	if len(sys.argv) == 1:
		print 'Please select the person you want to play with: Gavin or Mom'
		play_sound("Gavin", "SelectPerson")
		return
	else:
		if sys.argv[1].lower() == 'gavin':
			person = 'Gavin'
		elif sys.argv[1].lower() == 'mom':
			person = 'Mom'
		else:
			print 'Please select the person you want to play with: Gavin or Mom'
			play_sound("Gavin", "SelectPerson")
	...
	while 1:
		...
		while 1:
			...
			b.show()
			#winsound.PlaySound('C:\Users\Yu\Documents\Gavin\python\Gobang\gobang\sound\YourTurn.wma', winsound.SND_FILENAME)
			if (score > 7000):
				# Robot will win
				play_sound(person, "IGood")
			elif (score < -5000):
				play_sound(person, "YouGood")
			else:
				play_sound(person, "YourTurn")
